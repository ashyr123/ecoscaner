...
def seed_default_items() -> None:
    with get_connection() as conn:
        ...
        if c.fetchone()[0] == 0:
            c.executemany(
                """
                INSERT INTO store_items (name, description, price, stock, is_active)
                VALUES (?, ?, ?, ?, 1)
                """,
                [
...
def buy_item(telegram_id: int, item_id: int) -> tuple[bool, str]:
    ...
    with get_connection() as conn:
        ...
        row = c.fetchone()
        balance = row["total_points"] if row else 0

        if balance < price:
            missing = price - balance
            return False, (
                "❌ Недостаточно баллов на счету.\n"
                f"Нужно ещё: <b>{missing}</b> баллов."
            )
        ...
        c.execute(
            """
            INSERT INTO purchases (telegram_id, item_id, points_spent)
            VALUES (?, ?, ?)
            """,
    ...
    return True, (
        f"✅ Ты успешно получил: <b>{item['name']}</b>!\n"
